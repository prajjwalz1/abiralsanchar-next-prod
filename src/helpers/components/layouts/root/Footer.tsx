...
const Footer = () => {
  return (
    <footer className={`${colors.footer} footer pt-10 flex flex-col`}>
      <div
        className={`${styles.padding_x} footer-menu grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5`}
      >
      ...
      <div
        className={`${colors.footer_end} ${styles.padding_x} footer-end py-10 flex-1 flex flex-col lg:flex-row gap-5 lg:gap-1`}
      >
        <div className="logo">
          <Logo />
        </div>
        ...
        <div className="footer-end grid grid-cols-[auto_auto] md:grid-cols-[auto_auto_auto] lg:grid-cols-[auto_auto_auto_auto_auto] gap-4 items-center divide-x divide-gray-400">
          {footer_end_items.map(
            ({ label, icon, value }: OptionSchema, idx: number) => {
              ...
              const valueCss = isCopyright ? "text-sm" : "";
              return (
                <div
