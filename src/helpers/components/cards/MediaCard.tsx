...
export default function MediaCard(props: MediaCardSchema) {
  ...
  // Css
  const titleCss = `${fonts.slight_large_line_height} line-clamp-3`;

  return (
    <div className="h-[80px] flex gap-3 justify-between items-start rounded-md">
      <CustomImage
        src={image1!}
        alt={title.slice(0, 18)}
        divCss={`${fonts.span} font-medium w-[120px] h-full`}
      ...
      <div className="h-full flex-1 flex flex-col justify-between gap-1">
        <CustomText slug={slug} css={titleCss}>
          {title}
        </CustomText>
